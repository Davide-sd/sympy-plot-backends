...
def make_plot3d_parametric_line_1(B, rendering_kw, show=False):
    ...
    return plot3d_parametric_line(
        ...
        rendering_kw=rendering_kw, use_latex=False, **opts)
...
def make_plot3d_1(B, rendering_kw, show=False):
    ...
    return plot3d(
        ...
        use_cm=False, backend=B, rendering_kw=rendering_kw, **opts)
...
def make_plot3d_wireframe_2(B, rk):
    return plot3d(cos(x**2 + y**2), (x, -2, 2), (y, -2, 2),
        use_cm=True, backend=B,
        wireframe=True, wf_n1=20, wf_n2=30,
        wf_rendering_kw=rk, wf_npoints=12, **options())
...
def make_plot3d_wireframe_3(B, wf):
    ...
    return plot3d(
        ...
        backend=B, is_polar=True, use_cm=True, legend=True,
        color_func=lambda x, y, z: (x**2 + y**2)**0.5,
        wireframe=True, wf_n1=20, wf_n2=40, wf_rendering_kw=wf, **options())
...
def make_plot3d_parametric_surface_wireframe_1(B, wf):
    ...
    z = v / 2 * sin(u / 2)
    return plot3d_parametric_surface(x, y, z, (u, 0, 2*pi), (v, -1, 1),
        backend=B, use_cm=True,
        wireframe=True, wf_n1=5, wf_n2=6,
        wf_rendering_kw=wf, **options())
...
def make_plot3d_parametric_surface_wireframe_2(B, wf):
    ...
    return plot3d_parametric_surface(
        x, y, z, ("u", 0, 2*np.pi), ("v", -1, 0),
        backend=B, use_cm=True,
        wireframe=wf, wf_n1=5, wf_n2=6, **options())


def make_plot_contour_1(B, rendering_kw):
    return plot_contour(
        cos(x ** 2 + y ** 2), (x, -3, 3), (y, -3, 3),
        backend=B, rendering_kw=rendering_kw, use_latex=False, **options())


def make_plot_contour_is_filled(B, is_filled):
    return plot_contour(
        cos(x ** 2 + y ** 2), (x, -3, 3), (y, -3, 3),
        backend=B, use_latex=False, is_filled=is_filled, **options())


def make_plot_vector_2d_quiver(B, contour_kw, quiver_kw):
    return plot_vector(
        Matrix([x, y]), (x, -5, 5), (y, -4, 4),
        backend=B, quiver_kw=quiver_kw, contour_kw=contour_kw,
...
def make_plot_vector_2d_streamlines_1(B, stream_kw, contour_kw):
    return plot_vector(
        Matrix([x, y]), (x, -5, 5), (y, -4, 4),
        backend=B, stream_kw=stream_kw, contour_kw=contour_kw,
        scalar=(x + y), streamlines=True, use_latex=False, **options())
...
def make_plot_vector_2d_streamlines_2(B, stream_kw, contour_kw):
    return plot_vector(
        ...
        backend=B, stream_kw=stream_kw, contour_kw=contour_kw,
        scalar=[(x + y), "test"],
        streamlines=True, use_latex=False, **options())
...
def make_plot_vector_3d_streamlines_1(B, stream_kw, show=False, kwargs=dict()):
    ...
    return plot_vector(
        ...
        streamlines=True, use_latex=False, **opts, **kwargs)
...
def make_test_real_imag(B, rendering_kw):
    return plot_real_imag(
        sqrt(x), (x, -5, 5), backend=B, rendering_kw=rendering_kw,
        use_latex=False, **options())
...
def make_test_plot_piecewise_single_series(B):
    return plot_piecewise(
        ...
        backend=B, use_latex=False, **options())
...
def make_test_vectors_3d_update_interactive(B):
    return plot_vector([a * z, b * y, c * x], (x, -5, 5), (y, -5, 5), (z, -5, 5),
        params = {a: (1, 0, 2), b: (1, 0, 2), c: (1, 0, 2)},
        streamlines = True, n=5, backend=B, show=False)


def make_test_aspect_ratio_2d_issue_11764(B, aspect="auto"):
    return plot_parametric(cos(x), sin(x), (x, 0, 2 * pi),
        backend=B, aspect=aspect, **options())
...
def make_test_backend_latex_labels_1(B, use_latex):
    x1 = symbols("x_1^2")
    return plot(cos(x1), (x1, -1, 1), backend=B,
        use_latex=use_latex, **options())
...
def make_test_backend_latex_labels_2(B, use_latex, show=False):
    ...
    return plot3d(
        ...
        use_latex=use_latex, **opts)


def make_test_plot_use_latex(B):
    return plot(
        sin(x), cos(x), backend=B, legend=True, use_latex=True, **options())
...
def make_test_plot_contour_use_latex(B):
    return plot_contour(
        cos(x ** 2 + y ** 2), (x, -3, 3), (y, -3, 3),
        backend=B, use_latex=True, **options())
...
def make_test_plot3d_use_latex(B, show=False):
    ...
    return plot3d(
        cos(x ** 2 + y ** 2), sin(x ** 2 + y ** 2),
        (x, -3, 3), (y, -3, 3),
        use_cm=True, backend=B, use_latex=True, **opts)
...
def make_test_plot_vector_2d_streamlines_custom_scalar_field_use_latex(B):
    return plot_vector(
        Matrix([x, y]), (x, -5, 5), (y, -4, 4),
        backend=B, scalar=(x + y), streamlines=True, use_latex=True,
        **options())

def make_test_plot_vector_2d_streamlines_custom_scalar_field_custom_label_use_latex(B):
    return plot_vector(
        Matrix([x, y]), (x, -5, 5), (y, -4, 4),
        backend=B, scalar=[(x + y), "test"], streamlines=True, use_latex=True,
        **options())
...
def make_test_plot_complex_use_latex_1(B):
    return plot_complex(
        cos(x) + sin(I * x), (x, -2, 2), use_latex=True, backend=B, **options())
...
def make_test_plot_real_imag_use_latex(B):
    return plot_real_imag(sqrt(x), (x, -3, 3),
        backend=B, use_latex=True, **options())
...
def make_test_plot3d_use_cm(B, use_cm, show=False):
    opts = options()
    opts["show"] = show
    return plot3d(cos(x**2 + y**2), (x, -1, 1), (y, -1, 1),
...
def make_test_plot_polar(B, pa=False):
    return plot_polar(
        ...
        backend=B, polar_axis=pa, aspect="equal", **options())
...
def make_test_plot_polar_use_cm(B, pa=False, ucm=False, cf=None):
    return plot_polar(
        ...
        use_cm=ucm, color_func=cf, **options())
...
def make_test_surface_color_func_1(B, col, show=False):
    ...
    return plot3d(
        cos(x**2 + y**2), (x, -3, 3), (y, -3, 3),
        backend=B, color_func=col, use_cm=True, **opts)
...
def make_test_surface_color_func_2(B, col, show=False):
    ...
    return plot3d_parametric_surface(
        *expr, (u, 0, 2 * pi), (v, 0, pi), use_cm=True,
        backend=B, color_func=col, **opts)
...
def make_test_line_color_func(B, col):
    return plot(cos(x), (x, -3, 3),
        backend=B, color_func=col, legend=True, **options())
...
def make_test_line_color_plot3d_parametric_line(B, lc, use_cm, show=False):
    ...
    opts["show"] = show
    return plot3d_parametric_line(
        cos(x), sin(x), x, (x, 0, 2*pi), line_color=lc, backend=B,
        use_cm=use_cm, **opts)
...
def make_test_surface_color_plot3d(B, sc, use_cm, show=False):
    ...
    return plot3d(
        ...
        surface_color=sc, backend=B, use_cm=use_cm, legend=True, **opts)
...
def make_test_plot3d_list_use_cm_False(B, is_point, is_filled=False):
    ...
    return plot3d_list(x, y, z,
        backend=B, is_point=is_point, is_filled=is_filled,
        use_cm=False, show=False)


def make_test_plot3d_list_use_cm_color_func(B, is_point, is_filled=False, cf=None):
    x = [0, 1, 2, 3, 4, 5]
    y = [5, 4, 3, 2, 1, 0]
    z = [1, 3, 2, 4, 6, 5]
