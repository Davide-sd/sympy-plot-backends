...
def test_plot_geometry_interactive(pi_options):
    ...
    p = plot_geometry(
        ...
        params={a: (0, -1, 1), b: (1, -1, 1), c: (2, 1, 2), d: (3, 3, 8)},
        aspect="equal",
        is_filled=False,
        use_latex=False,
        xlim=(-2.5, 5.5),
...
def test_plot_parametric(p_options):
    ...
    p = plot_parametric(
        x + 1, x, (x, -2, 2), "test", {"cmap": "Reds"}, **p_options)
    assert p[0].expr == (x + 1, x)
    assert p[0].ranges == [(x, -2, 2)]
    assert p[0].get_label(False) == "test"
...
def test_plot_implicit(p_options):
    ...
    # multiple expressions
    with warns(UserWarning, match="The provided expression"):
        p = plot_implicit(
            (x > 0, (x, -2, 2), (y, -3, 3)),
            ((x > 0) & (y < 0), (x, -10, 10), "test", {"color": "r"}),
            **p_options
        )
    assert isinstance(p[0], ImplicitSeries)
    assert p[0].ranges == [(x, -2, 2), (y, -3, 3)]
...
def test_plot_list(p_options):
    ...
    p = plot_list((xx1, yy1, "cos"), (xx2, yy2, "sin"), **p_options)
    assert len(p.series) == 2
    assert all(isinstance(t, List2DSeries) for t in p.series)
...
def test_plot_piecewise(p_options):
    ...
    assert (
        ...
        and np.allclose(s[1].list_y, -1)
        and (not s[1].is_filled)
    )
    assert (
        (s[2].expr == x)
    ...
    assert (
        ...
        and np.allclose(s[3].list_y, -1e-06)
        and (not s[3].is_filled)
    )
    assert (
        (s[4].expr == x**2)
    ...
    assert (
        ...
        and np.allclose(s[5].list_y, 1 - 2e-06)
        and (not s[5].is_filled)
    )
    assert (
        (s[6].expr == x**3)
    ...
    assert (
        np.allclose(s[8].list_x, 0) and np.allclose(s[8].list_y, 0)
        and s[8].is_filled
    )
    assert (
        np.allclose(s[9].list_x, 1) and np.allclose(s[9].list_y, 1)
        and s[9].is_filled
    )
    ...
    assert (
        ...
        and np.allclose(s[2].list_y, 1)
        and (not s[2].is_filled)
    )
    assert (
        np.allclose(s[3].list_x, 0) and np.allclose(s[3].list_y, 0)
        and s[3].is_filled
    )
    ...
    assert len(
        [t for t in s if isinstance(t, List2DSeries) and t.is_filled]) == 2
    ...
    raises(
        NotImplementedError,
        lambda: plot_piecewise(f, (x, -10, 10), **p_options))
...
def test_plot_and_save_1(paf_options, adaptive):
    ...
    with TemporaryDirectory(prefix="sympy_") as tmpdir:
        ...
        p = plot(
            Piecewise((x, x < 1), (x**2, True)), (x, -3, 3), **pa_options)
...
def test_plot_and_save_2(paf_options, adaptive):
    ...
    with TemporaryDirectory(prefix="sympy_") as tmpdir:
        ...
        p = plot_parametric(
            sin(x), cos(x), (x, -5, 5), legend=True, **paf_options)
...
def test_plot_and_save_6(paf_options, adaptive):
    ...
    with TemporaryDirectory(prefix="sympy_") as tmpdir:
        filename = "test.png"
        #
        # Test expressions that can not be translated to np and generate
        # complex results.
        #
...
def test_issue_15265(paf_options, adaptive):
    ...
    raises(
        ValueError,
        lambda: plot(
            eqn, xlim=(-1, 1), ylim=(-1, S.ImaginaryUnit), **paf_options),
    )

    raises(
        ValueError,
        lambda: plot(
            eqn, xlim=(S.NegativeInfinity, 1), ylim=(-1, 1), **paf_options),
    )
...
def test_plot_implicit_label_rendering_kw(paf_options):
    ...
    p = plot_implicit(
        ...
        (x, -5, 5), (y, -5, 5),
        label=["a", "b"],
        rendering_kw=[{"levels": 5}, {"alpha": 0.5}],
        **paf_options
    )
...
def test_plot_implicit_adaptive_true():
    ...
    def do_test(expr, range_x, range_y, filename, tol=1):
        ...
        p = plot_implicit(
            expr, range_x, range_y,
            size=(5, 4),
            adaptive=True, grid=False, show=False, use_latex=False,
            backend=MB,
        )
        assert p.xlim and p.ylim
    ...
    try:
        ...
        do_test(
            y >= 2 * sin(x) * cos(x), (x, -5, 5), (y, -5, 5), "pi_05.png", 1)
        do_test(y <= x**2, (x, -5, 5), (y, -5, 5), "pi_06.png", 1)
        do_test(y > x, (x, -5, 5), (y, -5, 5), "pi_07.png", 5)
        do_test(
            And(y > exp(x), y > x + 2), (x, -5, 5), (y, -5, 5), "pi_08.png", 1)
        ...
        do_test(
            And(y > cos(x), Or(y > x, Eq(y, x))),
            (x, -5, 5), (y, -5, 5), "pi_13.png", 1
        )
        ...
        with warns(UserWarning, match="Adaptive meshing could not be applied"):
            do_test(
                Eq(y, re(cos(x) + I * sin(x))), (x, -5, 5), (y, -5, 5),
                "pi_15.png", 1
            )
    finally:
...
def test_number_discretization_points():
    ...
    assert p[0].n[0] == 10
    p = plot_parametric(
        cos(x), sin(x), (x, 0, 2 * pi), nb_of_points=10, **options)
    assert p[0].n[0] == 10
    p = plot_parametric(
        (cos(x), sin(x)), (cos(x), x), (x, 0, 2 * pi), n=10, **options)
    assert (len(p.series) > 1) and all(t.n[0] == 10 for t in p.series)

    p = plot_contour(cos(x**2 + y**2), (x, -pi, pi), (y, -pi, pi), **options)
    assert p[0].n[:2] == [100, 100]
    p = plot_contour(
        cos(x**2 + y**2), (x, -pi, pi), (y, -pi, pi), n=400, **options)
    assert p[0].n[:2] == [400, 400]
    p = plot_contour(
        cos(x**2 + y**2), (x, -pi, pi), (y, -pi, pi), n1=50, **options)
    ...
    assert p[0].n[:2] == [100, 100]
    p = plot_implicit(y > x**2, (x, -5, 5), (y, -10, 10), n=400, **options)
    assert p[0].n[:2] == [400, 400]
    p = plot_implicit(y > x**2, (x, -5, 5), (y, -10, 10), n1=50, **options)
    assert p[0].n[:2] == [50, 100]
    p = plot_implicit(
        y > x**2, (x, -5, 5), (y, -10, 10), n1=50, n2=20, **options)
    assert p[0].n[:2] == [50, 20]
    p = plot_implicit(y > x**2, (x, -5, 5), (y, -10, 10), points=20, **options)
    assert p[0].n[:2] == [20, 20]
    p = plot_implicit(
        y > x**2, y < sin(x), (x, -5, 5), (y, -10, 10), n=400, **options
    )
    assert (len(p.series) > 1) and all(t.n[:2] == [400, 400] for t in p.series)
