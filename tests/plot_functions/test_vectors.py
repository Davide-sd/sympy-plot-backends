...
@pytest.fixture
def pv_options_2d(panel_options):
...
@pytest.fixture
def pv_options_3d(panel_options):
    panel_options["backend"] = MB
    panel_options["n"] = 4
    return panel_options
...
def test_get_seeds_points():
    ...
    # domain
    # -------------------------------------------------------------------------
    d = get_seeds_points(xx, yy, zz, uu, vv, ww, True, **dict(starts=None))
    assert isinstance(d, np.ndarray)
    assert len(d.shape) == 2 and (d.shape[1] == 3)

    d = get_seeds_points(xx, yy, zz, uu, vv, ww, False, **dict(starts=None))
    ...
    d = get_seeds_points(
        xx, yy, zz, uu, vv, ww, True,
        **dict(starts={"x": xx2, "y": yy2, "z": zz2})
    )
    assert isinstance(d, np.ndarray)
    ...
    # -------------------------------------------------------------------------
    d = get_seeds_points(
        xx, yy, zz, uu, vv, ww, True, **dict(starts=True, npoints=10))
    assert isinstance(d, np.ndarray)
    assert len(d.shape) == 2 and (d.shape == (10, 3))

    d = get_seeds_points(
        xx, yy, zz, uu, vv, ww, False, **dict(starts=True, npoints=10))
    assert isinstance(d, vtk.vtkPointSource)


@pytest.mark.filterwarnings("ignore::UserWarning")
def test_plot_vector_2d(pv_options_2d):
    ...
    p = plot_vector([-sin(y), cos(x)], (x, -3, 2), (y, -4, 6), **pv_options_2d)
    s = p.series
    ...
    p = plot_vector(
        [-sin(y), cos(x)],
        (x, -3, 2), (y, -4, 6),
        scalar=lambda xx, yy: np.cos(xx * yy),
        **pv_options_2d
    )
    s = p.series
    assert isinstance(p, MB)
...
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_plot_vector_2d_interactive(pv_options_2d):
    # verify that plot_vector is capable of creating data
    # series according to the documented modes of operation when dealing
    # with 2D vectors
...
def test_plot_vector_3d(pv_options_3d):
    # verify that plot_vector is capable of creating data
    # series according to the documented modes of operation when dealing
...
@pytest.mark.skipif(pn is None, reason="panel is not installed")
def test_plot_vector_3d_interactive(pv_options_3d):
    # verify that plot_vector is capable of creating data
    # series according to the documented modes of operation when dealing
    # with 3d vector fields
...
def test_plot_vector_3d_slice(pv_options_3d):
    # verify that plot_vector is capable of creating data
    # series according to the documented modes of operation when dealing
    ...
    ss = ParametricSurfaceSeries(
        u * cos(v), u * sin(v), u, (u, -2, 0), (v, 0, 2 * pi), n1=5, n2=5
    )
    p = plot_vector(
        [z, y, x], (x, -10, 9), (y, -8, 7), (z, -6, 5), slice=ss,
        **pv_options_3d
    )
    s = p.series
    assert isinstance(p, MB)
    ...
    ss = SurfaceOver2DRangeSeries(
        cos(z**2 + y**2), (z, -4, 4), (y, -2, 2), n1=5, n2=5
    )
    p3a = plot_vector(
        [z, y, x], (x, -10, 9), (y, -8, 7), (z, -6, 5), slice=ss,
        **pv_options_3d
    ...
    ss = SurfaceOver2DRangeSeries(
        cos(x**2 + y**2), (x, -4, 4), (y, -2, 2), n1=5, n2=5
    )
    p3b = plot_vector(
        [z, y, x], (x, -10, 9), (y, -8, 7), (z, -6, 5), slice=ss,
        **pv_options_3d
...
@pytest.mark.skipif(pn is None, reason="panel is not installed")
def test_plot_vector_3d_slice_interactive(pv_options_3d):
    # verify that plot_vector is capable of creating data
    # series according to the documented modes of operation when dealing
    # with 3d vector fields and slices
    ...
    assert len(s) == 3
    assert all(
        isinstance(t, SliceVector3DSeries) and t.is_interactive for t in s)
    assert all(isinstance(t.slice_surf_series, PlaneSeries) for t in s)
    xx, yy, zz, _, _, _ = s[0].get_data()
    assert (xx.min(), xx.max()) == (-10, -10)
    assert (yy.min(), yy.max()) == (-8, 7)
    ...
    assert isinstance(p1b, spb.interactive.panel.InteractivePlot)
    assert len(s) == 1
    assert isinstance(s[0], SliceVector3DSeries) and s[0].is_interactive
    assert (
        isinstance(s[0].slice_surf_series, SurfaceOver2DRangeSeries)
        and s[0].slice_surf_series.is_interactive
    )
    d1b = s[0].get_data()
    xx, yy, zz, _, _, _ = d1b
    ...
    assert isinstance(p2b, spb.interactive.panel.InteractivePlot)
    assert len(s) == 1
    assert isinstance(s[0], SliceVector3DSeries) and s[0].is_interactive
    assert (
        isinstance(s[0].slice_surf_series, SurfaceOver2DRangeSeries)
    ...
    ss = SurfaceOver2DRangeSeries(
        cos(t * x**2 + y**2), (x, -4, 5), (y, -3, 2),
        n1=25, n2=25, params={t: 1}
    )
    p = plot_vector(
        [t * z, y, x],
        (x, -10, 9), (y, -8, 7), (z, -6, 5),
...
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_label_kw(pv_options_2d, pv_options_3d):
    ...
    # 2 series -> 2 labels
    p = plot_vector(
        [-sin(y), cos(x)], (x, -3, 3), (y, -3, 3), label=["a", "b"],
        **pv_options_2d
    )
...
@pytest.mark.skipif(pn is None, reason="panel is not installed")
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_label_kw_interactive(pv_options_2d, pv_options_3d):
    # verify that the label keyword argument works, if the correct
    # number of labels is provided.
...
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_rendering_kw(pv_options_2d, pv_options_3d):
...
@pytest.mark.skipif(pn is None, reason="panel is not installed")
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_rendering_kw_interactive(pv_options_2d, pv_options_3d):
    # verify that the rendering_kw keyword argument works, if the correct
    # number of dictionaries is provided, and that it will ovveride the values
    # into quiver_kw, stream_kw, contour_kw
...
def test_plot_vector_lambda_functions(pv_options_3d):
    ...
    # the same results
    p1 = plot_vector(v1, (x, -5, 5), (y, -2, 2), **pv_options_3d)
    p2 = plot_vector(
    ...
    assert all(
        np.allclose(t1, t2) for t1, t2 in
    ...
    p3 = plot_vector(
        v1, (x, -5, 5), (y, -2, 2),
        scalar=sqrt(x**2 + y**2), **pv_options_3d
    )
    p4 = plot_vector(
        [lambda x, y: x, lambda x, y: y],
        ("x", -5, 5),
...
@pytest.mark.skipif(pn is None, reason="panel is not installed")
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_parametric_ranges_2d_quiver(pv_options_2d):
    ...
    p = plot_vector(
        ...
        prange(x, -2 * a, 2 * b),
        prange(y, -2 * b, 2 * a),
        params={u: (1, 0, 2), a: (1, 0, 2), b: (1, 0, 2)},
        **pv_options_2d
    )
...
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_number_discretization_points(p_options):
    # verify the different ways of setting the numbers of discretization points
    x, y, z = symbols("x, y, z")

    p = plot_vector([-cos(y), sin(x)], (x, -10, 10), (y, -10, 10), **p_options)
    assert p[0].n[:2] == [100, 100]
    assert p[1].n[:2] == [25, 25]
    p = plot_vector(
        [-cos(y), sin(x)], (x, -10, 10), (y, -10, 10), n=15, nc=200,
        **p_options)
    assert p[0].n[:2] == [200, 200]
    assert p[1].n[:2] == [15, 15]
    p = plot_vector(
        [-cos(y), sin(x)], (x, -10, 10), (y, -10, 10), n1=15, n2=20, nc=50,
        **p_options)
    assert p[0].n[:2] == [50, 50]
    assert p[1].n[:2] == [15, 20]

    p = plot_vector(
        Matrix([cos(z), -cos(y), sin(x)]),
