...
def test_vector_field_2d(default_range, e1, e2, r1, r2, label, quiver_kw,
    contour_kw, stream_kw, streamlines, scalar, params):
    kwargs = {}
    if params: kwargs["params"] = params
    if quiver_kw: kwargs["quiver_kw"] = quiver_kw
    if stream_kw: kwargs["stream_kw"] = quiver_kw
    if contour_kw: kwargs["contour_kw"] = contour_kw
    print(e1, e2)
...
def test_vector_field_3d(default_range, e1, e2, e3, r1, r2, r3, label,
...
def test_vector_field_3d_sliced_series():
    ...
    vf = vector_field_3d(
        ...
        slice=cone_surface, use_cm=False, quiver_kw={"scale": 0.5, "pivot": "tail"}
    )
    assert len(vf) == 1
