...
colorcet_cms = [getattr(cc, t) for t in dir(cc) 
        if (t[0] != "_") and isinstance(getattr(cc, t), list)]
get_cms = lambda module: [getattr(module, a) for a in dir(module) 
        if ((a[0] != "_") and isinstance(getattr(module, a), list) 
            and all([isinstance(e, (float, int)) for e in getattr(module, a)]))]
k3d_cms = (get_cms(k3d.basic_color_maps) + 
    ...
    def do_test(colormaps, same=False):
        if not same:
            for c in colormaps:
                ...
                assert all([isinstance(t[1], str) and
                    ((t[1][:3] == "rgb") or (t[1][0] == "#")) for t in r])
        else:
            for c in colormaps:
