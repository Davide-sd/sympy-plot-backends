...
from spb.series import (
    ...
    _set_discretization_points, InteractiveSeries
)
...
def _build_series(*args, interactive=False, **kwargs):
    ...
    all_ranges = []
    for a in args:
        split_expr, ranges, s = _series(a[0], *a[1:-1], label=a[-1], 
    ...
    if all([s.is_2Dvector for s in series]):
        ...
        if scalar_field:
            ...
            cs_kwargs["n2"] = nc
            if not interactive:
                cs = ContourSeries(scalar_field, *cranges, scalar_label, 
                    **cs_kwargs)
            else:
                cs = InteractiveSeries([scalar_field], cranges, scalar_label, 
                    **cs_kwargs)
            series = [cs] + series
    ...
    if split_expr[2] is S.Zero: # 2D case
        ...
        if not interactive:
            return split_expr, ranges, Vector2DSeries(*split_expr[:2], *ranges, 
                    label, **kwargs)
        return split_expr, ranges, InteractiveSeries(split_expr[:2], ranges, 
                label, **kwargs)
    else: # 3D case
        ...
        if _slice is None:
            if not interactive:
                return split_expr, ranges, Vector3DSeries(*split_expr, *ranges,
                    label, **kwargs)
            return split_expr, ranges, InteractiveSeries(split_expr, ranges,
                 label, **kwargs)
        ...
        for s in _slice:
            if not interactive:
                ...
                series.append(InteractiveSeries(
                    split_expr, ranges, label, slice=s, **kwargs))
...
def _preprocess(*args, matrices=False, fill_ranges=True):
    ...
    new_args = []
    for a in args:
        exprs, ranges, label = _unpack_args(*a, matrices=matrices, 
