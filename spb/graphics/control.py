...
def _preprocess_system(system, **kwargs):
    ...
    if isinstance(system, (list, tuple)):
        if len(system) == 2:
            ...
            raise ValueError("If a tuple/list is provided, it must have "
...
def _unpack_systems(systems):
    ...
    if isinstance(systems[0], dict):
        ...
        all(all(isinstance(e, (int, float, Expr)) for e in t[:2]) for t in systems)
    ):
...
def pole_zero(
    ...
    **kwargs :
    ...
    """
    system = _preprocess_system(system)
    series = _pole_zero_helper(system, label, False,
...
def _step_response_helper(system, label, lower_limit, upper_limit,
    prec, **kwargs):
    system = _preprocess_system(system, **kwargs)
    _check_system(system)
...
def plot_step_response(*systems, lower_limit=0, upper_limit=10,
    ...
    **kwargs :
    ...
    systems = _unpack_systems(systems)
    series = [_step_response_helper(s, l,
...
def bode_magnitude(
    ...
    **kwargs :
    ...
    system = _preprocess_system(system)
    return [
        _bode_magnitude_helper(system, label, initial_exp, final_exp,
...
def bode_phase(
    ...
    **kwargs :
    ...
    system = _preprocess_system(system)
    return [
        _bode_phase_helper(system, label, initial_exp, final_exp,
