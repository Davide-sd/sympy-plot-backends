...
def modify_graphics_doc(replace={}, exclude=[], priority=[], style="numpydoc"):
    ...
    3. Take the 'PlotAttributes' class and extract its parameters.
    ...
    def decorator(func):
        ...
        original_docstring = original_docstring.split("\n")
        search_for = [red, blue, green, cyan, "Parameters of"]
        for i, line in enumerate(original_docstring):
            if any(t in line for t in search_for):
                break

        original_docstring = "\n".join(original_docstring[:i])
        ...
        for title in numpydoc_sections_to_look_for:
            if title != "Parameters":
                ...
                # extract PlotAttributes parameters
...
def modify_graphics_series_doc(series, replace={}, exclude=[], style="numpydoc"):
    """
